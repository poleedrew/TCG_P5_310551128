...
public:
	static int move_check(bool print = false) {
		board b;
		int n = 0;
		for (std::string m; std::cin >> m; ) {
			action::place a;
			if (m.find(';') == 0) { // in SGF string
				std::stringstream(m) >> a;
			} else { // in GTP string
				a = {board::point(m), b.info().who_take_turns};
			}
			if (a.apply(b) != board::legal) return b.info().who_take_turns;
			if (print) {
				const char* color[] = {"", "black", "white", ""};
				std::cerr << "Move #" << (++n) << ": "
					<< color[a.color()] << " at " << a.position() << std::endl << b;
			}
		}
		for (int i = 0; i < 81; i++) {
			if (board(b).place(i) == board::legal) return b.info().who_take_turns;
		}
		if (print) std::cout << b;
		return 0;
	}
